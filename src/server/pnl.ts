...
// ─── One-shot supplier data write ────────────────────────────────────────────

const SUPPLIER_INVOICES_DATA: unknown[][] = [
  ["SUPPLIER INVOICES", "", "", "", "", ""],
  ["Date", "Amount", "Units", "Shipping Method", "TA Order Number", "Note"],
  ["2026-08-05", 230.89, 1, "standard", "309979087001026047", "first supplier order, pre-negotiation pricing"],
  ["2026-08-09", 231.42, 2, "standard", "310471967001026047", "2-unit, direct-to-US, import=$14.70"],
  ["2026-08-13", 120.09, 1, "standard", "311726258501026047", "direct-to-US, import=$6.60"],
  ["2026-08-17", 122.64, 1, "standard", "312115942501026047", "direct-to-US, import=$9.07"],
  ["2026-08-20", 113.29, 1, "standard", "311590951501026047", ""],
  ["2026-08-20", 139.04, 1, "standard", "312840954001026047", "unit + spare cartridge"],
  ["2026-08-20", 113.29, 1, "standard", "311747747001026047", ""],
  ["2026-08-22", 113.29, 1, "standard", "313033578001026047", ""],
  ["2026-08-22", 113.29, 1, "standard", "312795146501026047", ""],
  ["2026-08-22", 139.04, 1, "standard", "313001138001026047", "unit + spare cartridge"],
  ["2026-08-24", 113.29, 1, "standard", "312240099001026047", ""],
  ["2026-08-25", 113.29, 1, "standard", "312383679001026047", ""],
  ["2026-08-25", 252.33, 2, "standard", "312097543501026047", "2 units + spare cartridge"],
  ["2026-08-25", 113.29, 1, "standard", "312334895001026047", ""],
  ["2026-08-26", 113.29, 1, "standard", "312514339001026047", ""],
  ["2026-08-26", 113.29, 1, "standard", "312239327501026047", ""],
  ["2026-08-29", 113.29, 1, "standard", "313856646001026047", ""],
  ["2026-08-31", 32.45, 0, "standard", "313066955001026047", "spares kit x1"],
  ["2026-09-02", 113.29, 1, "standard", "313032463501026047", ""],
  ["2026-09-02", 226.58, 2, "standard", "313048831501026047", "2-unit order"],
  ["2026-09-03", 20.60, 0, "standard", "314231982501026047", "spare cartridge x1"],
  ["2026-09-05", 128.74, 1, "express", "314478150501026047", ""],
  ["2026-09-07", 113.29, 1, "standard", "313637267501026047", ""],
];

export async function writeSupplierData(): Promise<{ rowsWritten: number; inputsUpdated: boolean }> {
  const sa = getServiceAccountKey();
  const spreadsheetId = getSheetId();
  const token = await getAccessToken(sa);

  // Write Supplier Invoices tab (overwrite from A1)
  await clearAndWrite(
    token,
    spreadsheetId,
    `'Supplier Invoices'!A1:F${SUPPLIER_INVOICES_DATA.length + 5}`,
    SUPPLIER_INVOICES_DATA,
  );

  // Update Inputs!B2 (standard landed cost) and B3 (express landed cost) and B4 (kit landed cost)
  // Use RAW input to prevent date parsing on the amounts
  const inputsUrl = `${SHEETS_BASE}/${spreadsheetId}/values/'Inputs'!B2:C4?valueInputOption=USER_ENTERED`;
  await fetch(inputsUrl, {
    method: "PUT",
    headers: {
      Authorization: `Bearer ${token}`,
      "Content-Type": "application/json",
    ...
    body: JSON.stringify({
      range: "'Inputs'!B2:C4",
      values: [
        [113.29, "Alibaba amount paid incl. processing fee"],
        [128.74, "Alibaba amount paid incl. processing fee"],
        [32.45, "Spares kit landed cost"],
      ],
    }),
  });

  return { rowsWritten: SUPPLIER_INVOICES_DATA.length - 2, inputsUpdated: true };
}
