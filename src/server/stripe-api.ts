...
import { runPnl, writeSupplierData } from "./pnl";
...
async function handlePnl(request: Request) {
  ...
  if (action === "write-suppliers") {
    try {
      const result = await writeSupplierData();
      return json({ ok: true, ...result });
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      console.error(JSON.stringify({ event: "pnl_supplier_write_error", message }));
      return json({ error: message }, { status: 500 });
    }
  }
